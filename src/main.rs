...
use futures::TryStreamExt; // 使用 TryStreamExt 提供的 try_concat 方法
...
fn get_host_port(host_name: &str) -> (&str, u16) {
    match host_name.find(":") {
        None => (host_name, 80),
        Some(i) => (&host_name[0..i], *&host_name[i + 1..].parse().unwrap_or(80)),
    }
}
                    ...
                    match resp {
                        None => {
                            ...
                            match remote_resp.await {
                                Ok(resp) => Ok::<_, hyper::Error>(intercept_response(resp).await),
                                Err(err) => {
                        ...
                        Some(resp) => Ok::<_, hyper::Error>(resp),
                    }
                }
            }),
        )
...
async fn https_remote_connect<B>(
    host: &str,
    port: u16,
) -> Result<(SendRequest<B>, Connection<TokioIo<SslStream<TcpStream>>, B>)>
where
    B: Body + 'static,
    B::Data: Send,
